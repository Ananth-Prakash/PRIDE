...
cor_values <- cor(tmp_intensities_ppb_normalised[-1])
colnames(cor_values) <- gsub("ppb.Intensity.", "", colnames(cor_values), perl=TRUE)
rownames(cor_values) <- gsub("ppb.Intensity.", "", rownames(cor_values), perl=TRUE)
...
Original dataset submitter:\hspace{7pt}  Sunil Adav  
E-mail:\hspace{94pt}                     ssadav@ntu.edu.sg  
PubMed ID:\hspace{73pt}                  30691479  

Quantification method:\hspace{25pt}      Differential  
Search database:\hspace{53pt}            Human SwissProt Proteome (UniProt, May 2019. 20,367 sequences)  
...
Replicates:\hspace{80pt}                 3 replicates per sample  
…\hspace{117pt}                          labels 114 and 116 for non-demented brain samples (controls).  
…\hspace{117pt}                          labels 115 and 117 for Alzheimer's disease brain samples.  
...
Figure 5.  Correlation between samples. The pairwise Pearson correlation was calculated between normalised intensities of each sample and clustered heirarchically.  
Sample name description: **AD**: Alzheimer's Disease; A, B and C represent replicates.  
...
ggplot(foldchange, aes(x=log2FC,  y = -log10(adj.pvalue), colour=threshold))+
  ...
  theme_bw()+
  theme(legend.position = "none")+
  xlab("log2 Fold Change (Non-demented / Alzheimers)")+
...
Figure 6.  Differential protein expression. Proteins which show statistically significant differential expression (p-value <= 0.05) are coloured red.  
