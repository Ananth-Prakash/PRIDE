---
title: "Summary statistics of post-processed MaxQuant output submitted to Expression Atlas."
output: pdf_document
---

## Summary

Summary of output from MaxQuant (proteinGroups.txt).  
PXDID: PXD004143  
Protein reference: Human Reference Proteome (UniProt, May 2019)  
Dataset: /Users/ananth/Documents/MaxQuant_Bechmarking/Human/PXD004143/proteinGroups.txt  
...
dir <- "/Users/ananth/Documents/MaxQuant_Bechmarking/Human/PXD004143/"
...
**†** The total number of unique peptides associated with the protein group (i.e. these peptides are not shared with another protein group).    
  * Protein groups consisting proteins to which 2 or more peptides are mapped.    
  • Protein groups consisting proteins which are mapped to an unique Ensembl Gene ID.
...
Boxplots with distribution of iBAQ values for each sample before and after FOT normalisation.  
```{r tmp_iBAQs_plot, fig.height = 11, echo = FALSE}
# iBAQ plot before and after normalisation
load(paste(dir,"iBAQ_plot.rda", sep=""))
ggplot(tmp_iBAQs_plot, aes(x=Samples, y = iBAQ, colour = Samples))+
  geom_boxplot()+
  theme_bw()+
...
ggplot(combined_protein_count_plot, aes(x=Samples, y = Protein_count, fill = Type))+
...
ggplot(Peptidecount_sample_plot, aes(x=Samples, y = Peptide_count_sum, fill = Type))+
  geom_col(position="dodge")+
  theme_bw()+
