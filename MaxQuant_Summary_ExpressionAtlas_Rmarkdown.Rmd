...
#Count number of SwissProt entries without isoforms
tmp$Sprot_counts <- "NA"
...
```
  **†** The total number of SwissProt proteins (non-isoforms) to which atleast 2 or more peptides from each sample are mapped to.  
...
```
Figure2. Protein overlap.  Indicates the number of protein groups that were identified across different samples. Protein groups can have more than one protein (isoforms included).  
