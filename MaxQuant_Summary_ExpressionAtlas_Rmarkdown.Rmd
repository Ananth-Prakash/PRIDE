...
Operating system:\hspace{46pt}           Red Hat Enterprise Linux Server
...
\setlength{\leftskip}{0.5cm}**†** The total number of SwissProt proteins (non-isoforms) to which at least 2 or more peptides from each sample are mapped to.  
...
```
Figure 3.  Protein counts in each sample. The total number of proteins (SwissProt non-isoforms) from all protein groups to which peptides from each sample are mapped.   
...
Figure 5.  Correlation between samples. The Pearson correlation between normalised intensities of each sample. The pairwise Pearson correlation was calculated between sample iBAQs.   
...
**Reverse decoy**: This particular protein group contains no protein, made up of at least 50% of the peptides of the leading protein, with a peptide derived from the reversed part of the decoy database. These should be removed for further data analysis. The 50% rule is in place to prevent spurious protein hits to erroneously flag the protein group as reverse.  

**Potential contaminant**: This particular protein group was found to be a commonly occurring contaminant. These should be removed for further data analysis.    
