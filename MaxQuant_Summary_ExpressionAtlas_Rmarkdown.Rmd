...
PRIDE dataset identifier:\hspace{15pt}   `r pride_id$value`  
PRIDE dataset:\hspace{58pt}              `r pride_link$value`  
Reference:\hspace{83pt}                  Human Reference Proteome (UniProt, May 2019)  
Number of sequences:\hspace{33pt}        95,915  
Software:\hspace{87pt}                   MaxQuant v1.6.3.4  
Operating System:\hspace{46pt}           Red Hat Enterprise Linux Server  

Original dataset submitter:\hspace{9pt} `r submitter_name$value`  
PubMed ID:\hspace{75pt}                 `r pubmed_id$value`  
E-mail:\hspace{97pt}                    `r submitter_email$value`  
...
ggplot(protein_overlap_freq[protein_overlap_freq$Protein.overlap != 0, ], aes(x=factor(Protein.overlap), y = freq))+
  ...
  theme(legend.position="none")+
  ggtitle("Proteins commonly identified across samples")
```
Figure 2. Protein overlap.  Indicates the number of protein groups that were identified across different samples. Protein groups were counted as present in a sample when the sample had registered intensity. Protein groups can have more than one protein within.  
...
ggplot(proteincount_table_post, aes(x=Samples, y = Protein_count_sum, fill=Samples))+
  ...
  #theme(axis.text.x = element_text(angle = 90, hjust = 1))+
  theme(legend.position="none")+
  ggtitle("Number of proteins idnetified in each sample")
...
**Reverse**: This particular protein group contains no protein, made up of at least 50% of the peptides of the leading protein, with a peptide derived from the reversed part of the decoy database. These should be removed for further data analysis. The 50% rule is in place to prevent spurious protein hits to erroneously flag the protein group as reverse.  
